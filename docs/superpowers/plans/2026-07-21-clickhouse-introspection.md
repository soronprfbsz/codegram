...
- 검증 명령은 codegram 도커 스택이 떠 있어야 동작한다(`deploy/scripts/start.sh`; 백엔드 4000·프론트 4001·pg 35432). ClickHouse 실 인스턴스: `10.140.1.40:8123`, user `hawkeye` / pw `hawkeye-test` / database `hawkeye`.
...
e = create_engine('clickhouse+http://hawkeye:hawkeye-test@10.140.1.40:8123/hawkeye')
with e.connect() as c:
...
curl -s -X POST http://localhost:4000/api/introspect \
  ...
  -d '{"dialect":"clickhouse","host":"10.140.1.40","port":8123,"username":"hawkeye","password":"hawkeye-test","database":"hawkeye"}' \
...
Expected: PASS. (spec 코드는 기존 db-import E2E의 셀렉터·로그인 헬퍼를 그대로 재사용해 작성 — `db-connect-dialect`에 `clickhouse` 선택, `db-connect-host`=`10.140.1.40`, `db-connect-port`=`8123`, `db-connect-username`/`password`/`database` 입력.)
