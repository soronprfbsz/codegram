...
describe('gridLayout', () => {
  it('lays out many isolated tables in a balanced grid (wider than tall), not one column', () => {
    ...
    // landscape: distinct columns >= distinct rows
    expect(xs.size).toBeGreaterThanOrEqual(ys.size)
  })
...
describe('getHelperLines', () => {
  ...
  it('aligns centers', () => {
    ...
    const dragged = node('d', 502, 300, 240, 100)    // centerX = 622, diff 2 → snap left so centerX=620 → left=500
    const r = getHelperLines(dragged, [other])
    expect(r.snapX).toBe(500)
