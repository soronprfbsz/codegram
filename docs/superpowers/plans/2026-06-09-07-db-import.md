...
Run:
```bash
docker compose -p codegram up -d --build backend
docker compose -p codegram exec -T backend python -c "import psycopg2, pymysql; print('drivers ok')"
```
Expected: `drivers ok`
...
Run: `docker compose -p codegram exec -T backend pytest tests/test_introspect_schemas.py -v`
...
Run: `docker compose -p codegram exec -T backend pytest tests/test_introspect_service.py -v`
...
Run: `docker compose -p codegram exec -T backend pytest tests/test_introspect_service.py -k build_ddl -v`
...
Run: `docker compose -p codegram exec -T backend pytest tests/test_introspect_service.py -k importable -v`
...
Run: `docker compose -p codegram exec -T backend pytest tests/test_introspect_routes.py -v`
...
docker compose -p codegram exec -T backend alembic upgrade head
docker compose -p codegram exec -T postgres sh -c 'echo ready'
```
Expected: 모든 컨테이너 up, `ready`.

- [ ] **Step 2: compose의 postgres 접속 좌표 확인**

Run: `docker compose -p codegram exec -T backend printenv | grep -i -E 'POSTGRES|DATABASE' || cat .env`
사용할 값 기록: DB 사용자/비밀번호/DB명(예: `postgres`/`postgres`/`codegram`), 그리고 백엔드 컨테이너에서 본 Postgres 호스트명은 compose 서비스명 `postgres`, 포트 `5432`. (이 좌표는 백엔드 컨테이너 기준 — introspect는 백엔드에서 일어나므로 컨테이너 네트워크 호스트명을 쓴다.)

- [ ] **Step 3: Write the E2E test**

`frontend/e2e/db-import.spec.ts` — `e2e/projects.spec.ts`의 로그인/네비게이션 규약을 미러링한다. **실제 내부 접속 좌표(확인됨):** host `postgres`, port `5432`, user `codegram_user`, password `postgres_dev`, database `codegram_dev`, schema `public`. (introspect는 백엔드 컨테이너에서 일어나므로 compose 서비스명 `postgres`를 호스트로 쓴다 — 호스트의 35432 포트가 아님.)
...
Run:
```bash
docker compose -p codegram exec -T backend pytest -q
