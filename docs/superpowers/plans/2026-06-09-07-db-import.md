...
def test_ssl_connect_args_per_dialect():
    ...
    # PyMySQL needs a truthy SSL value; an empty dict is falsy and silently
    # leaves the connection unencrypted. We pass an SSLContext that encrypts
    # without verifying (the "require" level chosen for postgres above).
...
`frontend/e2e/db-import.spec.ts` — `e2e/projects.spec.ts`의 로그인/네비게이션 규약을 미러링한다. **실제 내부 접속 좌표(확인됨):** host `postgres`, port `5432`, user `erddbml_user`, password `postgres_dev`, database `erddbml_dev`, schema `public`. (introspect는 백엔드 컨테이너에서 일어나므로 compose 서비스명 `postgres`를 호스트로 쓴다 — 호스트의 35432 포트가 아님.)

```ts
import { test, expect } from '@playwright/test'
// 기존 e2e 스펙(projects.spec.ts 등)의 로그인 방식을 그대로 따른다. 인라인 회원가입/로그인이면
// 그 단계를 복사하고, 공용 헬퍼가 있으면 그 헬퍼를 import 한다.

test('imports the backend Postgres schema into a new project', async ({ page }) => {
  // ── 기존 스펙과 동일하게 회원가입/로그인 후 홈(/)에 도달 ──
  // (projects.spec.ts의 로그인 단계를 복사)
...
> **참고:** 로그인 방식과 정확한 셀렉터는 `e2e/projects.spec.ts`를 열어 실제에 맞춘다. playwright `baseURL`이 현재 가동 중인 프론트 포트와 맞는지 확인한다(로컬 override로 프론트가 host 4001에 게시되어 있을 수 있음 — `playwright.config.ts`의 baseURL/`webServer` 설정과 대조).
