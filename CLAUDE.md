...
- **PostToolUse** (`lint_rules.py`): 편집 직후 규칙 위반을 **lint**해 피드백 — 프론트 FSD import 경계(상위 계층 import·widget→widget), 백엔드 얇은 라우트(라우트 내 ORM 직접 사용)·모델 변경 시 마이그레이션 확인. 비차단(정보 제공)이라 위반이면 수정, 의도/사전존재면 그 사실을 밝힌다.
